...
class IMU:
    def __init__(self):
        ...
        self.latest_linear_acceleration = {'x': 0.0, 'y': 0.0, 'z': 0.0}
        self.latest_angular_velocity = {'x': 0.0, 'y': 0.0, 'z': 0.0}
        self.pitch_angle = 0
        self.roll_angle = 0
        self.yaw_angle = 0
    ...
    def update_imu_state(self):
        ...
        filtered_accel_x = self.kalman_filter_AX.kalman(accel_data['x'] - self.error_accel_data['x'])
        filtered_accel_y = self.kalman_filter_AY.kalman(accel_data['y'] - self.error_accel_data['y'])
        filtered_accel_z = self.kalman_filter_AZ.kalman(accel_data['z'] - self.error_accel_data['z'])
        
        filtered_gyro_x = self.kalman_filter_GX.kalman(gyro_data['x'] - self.error_gyro_data['x'])
        filtered_gyro_y = self.kalman_filter_GY.kalman(gyro_data['y'] - self.error_gyro_data['y'])
        filtered_gyro_z = self.kalman_filter_GZ.kalman(gyro_data['z'] - self.error_gyro_data['z'])


        self.latest_linear_acceleration['x'] = filtered_accel_x
        self.latest_linear_acceleration['y'] = filtered_accel_y
        self.latest_linear_acceleration['z'] = filtered_accel_z
        
        self.latest_angular_velocity['x'] = filtered_gyro_x
        self.latest_angular_velocity['y'] = filtered_gyro_y
        self.latest_angular_velocity['z'] = filtered_gyro_z

        accel_x, accel_y, accel_z = filtered_accel_x, filtered_accel_y, filtered_accel_z
        gyro_x, gyro_y, gyro_z = filtered_gyro_x, filtered_gyro_y, filtered_gyro_z
